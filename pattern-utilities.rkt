(module pattern-utilities scheme
  (require "juggling-core.rkt" srfi/1)
  ...
  ; Use translate-hands and rotate-hands to pretend you're doing a pair of hands here...
  ; Until some future time when I unscrew basics of the juggler positions.  
  (define (pair-of-hands-1-segment init-offset seg-length pos-f)
    (list 
     (make-path-state init-offset #f 
                      (circular-list
                       (make-juggler-path-segment   
                        seg-length
                        (lambda _ 
                          (lambda (t) (car (pos-f t)))))))
